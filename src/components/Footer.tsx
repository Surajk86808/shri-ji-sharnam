    ...
    <footer id="contact" className="bg-luxury-deep text-white pt-24 md:pt-32 pb-12 px-6 md:px-8">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16 md:gap-24 mb-24 md:mb-32">
        ...
        <div className="space-y-8 md:space-y-10 text-center md:text-left">
          <h4 className="text-luxury-gold uppercase tracking-[0.3em] text-[10px] font-bold">{t.footer.explore}</h4>
          <ul className="space-y-4 md:space-y-5 text-white/50 font-light text-sm tracking-widest">
            <li><button onClick={() => scrollTo('home')} className="hover:text-white transition-colors">{language === 'hi' ? 'मुख्य पृष्ठ' : 'Home'}</button></li>
            <li><button onClick={() => scrollTo('experience')} className="hover:text-white transition-colors">{t.navbar.experience}</button></li>
            <li><button onClick={() => scrollTo('amenities')} className="hover:text-white transition-colors">{t.navbar.residences}</button></li>
            <li><button onClick={() => scrollTo('gallery')} className="hover:text-white transition-colors">{t.navbar.vrindavan}</button></li>
          </ul>
        </div>

        <div className="space-y-8 md:space-y-10 text-center md:text-left">
          <h4 className="text-luxury-gold uppercase tracking-[0.3em] text-[10px] font-bold">{t.footer.sanctuary}</h4>
          <ul className="space-y-6 md:space-y-8 text-white/50 font-light text-sm tracking-wide">
            ...
            <li className="flex flex-col md:flex-row items-center md:items-start gap-3 md:gap-5">
              ...
              <span>{t.footer.host} <br /> <span className="text-[10px] opacity-60">Replies within an hour</span></span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto pt-12 md:pt-16 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8 text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.4em] text-white/20 font-medium">
        <p className="text-center md:text-left">&copy; 2026 Shri Ji Sharnam. {t.footer.rights}</p>
